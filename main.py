...
async def send_reaction(client: TelegramClient, peer: str, message_id: int, emoji: str) -> bool:
    try:
...
async def react_to_message(link: str, emoji: str = REACTION_EMOJI, max_accounts: Optional[int] = None) -> Dict[str, Any]:
    parsed = parse_telegram_link(link)
    if not parsed:
        return {"status": "error", "detail": "Invalid Telegram message link"}
    ...
    if not accounts:
        return {"status": "error", "detail": "No accounts in account.json"}
    ...
    for idx, acc in enumerate(accounts):
        ...
        if not all([session_str, api_id, api_hash]):
            results.append({"account_index": idx, "status": "skipped", "reason": "Missing fields"})
            continue

        # 🔥 KEY FIX: Use StringSession — no filesystem write needed!
        string_session = sessions.StringSession(session_str)
        client = TelegramClient(string_session, api_id, api_hash)

        try:
            await client.start()
            logger.info(f"Account {idx}: connected")
            ...
            if success:
                success_count += 1
                results.append({"account_index": idx, "status": "success"})
            else:
                fail_count += 1
                results.append({"account_index": idx, "status": "failed"})

        except SessionPasswordNeededError:
            fail_count += 1
            results.append({"account_index": idx, "status": "failed", "reason": "2FA required"})
        except AuthKeyUnregisteredError:
            fail_count += 1
            results.append({"account_index": idx, "status": "failed", "reason": "Session expired"})
        except Exception as e:
            fail_count += 1
            results.append({"account_index": idx, "status": "failed", "reason": str(e)})
        finally:
...
async def lifespan(app: FastAPI):
    if not os.path.exists(ACCOUNT_FILE):
        ...
        logger.info(f"API ready with {len(load_accounts())} accounts")
    yield


app = FastAPI(title="Telegram Reaction API", version="1.0.0", lifespan=lifespan)


@app.get("/")
async def root():
    return {"message": "Telegram Reaction API running", "accounts": len(load_accounts())}


@app.get("/get")
async def get_reaction(
    link: str = Query(...),
    emoji: str = Query(REACTION_EMOJI),
